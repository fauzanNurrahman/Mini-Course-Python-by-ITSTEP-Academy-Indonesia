...
# 2. Python berinteraksi dengan broowser
...
wb = load_workbook(filename="web automation/Data.xlsx")
# print(wb)
...
while index <= len(sheetRange["A"]):
    ...
    # Handle add button
    add_bttn = driver.find_element(By.ID, "addNewRecordButton")
    add_bttn.click()

    # check condition using try
    try:
        WebDriverWait(driver, 10).until(EC.visibility_of_all_elements_located((By.ID, "registration-form-modal")))
    ...
    time.sleep(1)
    print(f"Data ke-{index} yaitu {first_name} terinout")
    index = index + 1
